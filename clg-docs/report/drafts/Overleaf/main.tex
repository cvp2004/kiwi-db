...
% Suppress page numbering on the unnumbered front matter
% (Title, Certificate, Acknowledgement, Abstract).
% Overridden by \pagestyle{frontmatter} once \frontmatter begins.
\pagestyle{empty}
...
% \noindent
% \begin{longtable}{|p{2cm}|p{10.3cm}|p{1.5cm}|}
% \hline
...
% \hline
% \endlastfoot
% 4.1 & KiwiDB System Architecture & \pageref{fig:sysarch} \\
% \hline
% 4.2 & KiwiDB Layered Dependency Model & \pageref{fig:arch} \\
% \hline
% 4.3 & DFD Level 0 --- Context Diagram & \pageref{fig:dfd0} \\
% \hline
% 4.4 & DFD Level 1 --- Major Subsystems and Data Stores & \pageref{fig:dfd1} \\
% \hline
% 4.5 & DFD Level 2 --- Write Path Detail & \pageref{fig:dfd2-write} \\
% \hline
% 4.6 & DFD Level 2 --- Read Path Detail & \pageref{fig:dfd2-read} \\
% \hline
% 4.7 & Use Case Diagram for KiwiDB & \pageref{fig:usecase} \\
% \hline
% 4.8 & Activity Diagram --- Write Path & \pageref{fig:activity} \\
% \hline
% 4.9 & Activity Diagram --- Read Path & \pageref{fig:activity-read} \\
% \hline
% 4.10 & Sequence Diagram --- Put, Get, and Flush Operations & \pageref{fig:sequence} \\
% \hline
% 4.11 & Class Diagram --- Core Class Hierarchy & \pageref{fig:classdiag} \\
% \hline
% 4.12 & Component Diagram & \pageref{fig:component} \\
% \hline
...
% \hline
% 8.1  & Project Documentation Website --- QR Code & \pageref{fig:qr-docs} \\
% \hline
% 8.2  & Web Dashboard --- Home page & \pageref{fig:screen-dashboard-home} \\
% \hline
% 8.3  & Web Dashboard --- KV Explorer page & \pageref{fig:screen-kv-explorer} \\
% \hline
% 8.4  & Web Dashboard --- MemTable Viewer page & \pageref{fig:screen-memtable-viewer} \\
% \hline
% 8.5  & Web Dashboard --- SSTable Browser page & \pageref{fig:screen-sstable-browser} \\
% \hline
% 8.6  & Web Dashboard --- Compaction Monitor page & \pageref{fig:screen-compaction-monitor} \\
% \hline
% 8.7  & Web Dashboard --- Live Log Stream page & \pageref{fig:screen-live-logs} \\
% \hline
% 8.8  & Web Dashboard --- In-Browser Terminal page & \pageref{fig:screen-terminal} \\
% \hline
% 8.9  & Web Dashboard --- Configuration page & \pageref{fig:screen-config} \\
% \hline
% 8.10 & Web Dashboard --- Stats and Write-Amplification page & \pageref{fig:screen-stats} \\
% \hline
% 8.11 & CLI REPL --- interactive session & \pageref{fig:screen-repl} \\
% \hline
% 8.12 & Automated test suite --- \texttt{pytest} output & \pageref{fig:screen-pytest} \\
% \hline
% 8.13 & Static type checking --- \texttt{mypy} and \texttt{basedpyright} & \pageref{fig:screen-typecheck} \\
% \hline
% \end{longtable}
...
% \hline
% 6.2 & Tools and Technologies used & \pageref{tab:tech} \\
% \hline
...
% Override chapter format for LoF so title and content appear on same page
{
\titleformat{\chapter}
{\fontsize{14}{17}\filcenter\bfseries}
{}
...
% Override chapter format for LoT so title and content appear on same page
{
\titleformat{\chapter}
{\fontsize{14}{17}\filcenter\bfseries}
{}
...
KiwiDB is structured as a stack of cooperating layers. External clients enter through a thin HTTP / WebSocket presentation tier; a single \texttt{LSMEngine} coordinator delegates to five manager subsystems, each of which owns a set of low-level workers (skip list, WAL writer, SSTable reader/writer, bloom filter, block cache, merge iterator); persistence is handled by the local filesystem. Figure~\ref{fig:sysarch} shows this top-to-bottom view, and Figure~\ref{fig:arch} complements it with the strict module-level dependency hierarchy that enforces it.
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    client/.style={draw, rounded corners, fill=blue!10, minimum width=2.7cm, minimum height=0.9cm, font=\scriptsize, align=center},
    api/.style={draw, rounded corners, fill=purple!15, minimum width=13cm, minimum height=1cm, font=\scriptsize, align=center},
    engine/.style={draw, rounded corners, fill=orange!20, minimum width=7cm, minimum height=1cm, font=\scriptsize, align=center, thick},
    mgr/.style={draw, rounded corners, fill=green!15, minimum width=1.95cm, minimum height=0.9cm, font=\scriptsize, align=center},
    worker/.style={draw, rounded corners, fill=gray!15, minimum width=1.95cm, minimum height=0.8cm, font=\scriptsize, align=center},
    fs/.style={draw, rounded corners, fill=white, minimum width=2.0cm, minimum height=0.7cm, font=\scriptsize, align=center},
    arr/.style={-{Stealth[length=2mm]}, thick, gray!70},
    arrk/.style={-{Stealth[length=2mm]}, thick, black},
    band/.style={draw=none, font=\scriptsize\itshape, text=black, anchor=west},
]
% ── Row 1: Clients
\node[client] (repl)   at (-4.5, 9) {REPL\\\scriptsize prompt-toolkit};
\node[client] (spa)    at (-1.5, 9) {React SPA\\\scriptsize web dashboard};
\node[client] (httpc)  at ( 1.5, 9) {HTTP client\\\scriptsize \texttt{curl}, \texttt{requests}};
\node[client] (embed)  at ( 4.5, 9) {Embedded\\\scriptsize \texttt{await LSMEngine}};

% ── Row 2: Presentation
\node[api] (fastapi) at (0, 7.3) {\textbf{FastAPI + Uvicorn (ASGI)} \quad \textbf{WebSocket}};

% ── Row 3: Engine core
\node[engine] (engine) at (0, 5.6) {\textbf{LSMEngine} \ --- \ put / get / delete / flush / close \ (async)};

% ── Row 4: Managers
\node[mgr] (walmgr)  at (-5.5, 4) {WAL\\Manager};
\node[mgr] (memmgr)  at (-3.3, 4) {MemTable\\Manager};
\node[mgr] (sstmgr)  at (-1.1, 4) {SSTable\\Manager};
\node[mgr] (flushp)  at ( 1.1, 4) {Flush\\Pipeline};
\node[mgr] (compmgr) at ( 3.3, 4) {Compaction\\Manager};
\node[mgr] (conf)    at ( 5.5, 4) {Config /\\SeqGen};

% ── Row 5: Workers
\node[worker] (walw)  at (-5.5, 2.4) {WALWriter};
\node[worker] (skip)  at (-3.3, 2.4) {SkipList\\Active / Imm};
\node[worker] (sstrw) at (-1.1, 2.4) {SSTable\\Writer / Reader};
\node[worker] (bloom) at ( 1.1, 2.4) {BloomFilter\\SparseIndex};
\node[worker] (cache) at ( 3.3, 2.4) {BlockCache};
\node[worker] (merge) at ( 5.5, 2.4) {KWayMerge\\Iterator};

% ── Row 6: Filesystem
\node[fs] (wal)  at (-5.5, 0.8) {\texttt{wal.log}};
\node[fs] (ssts) at (-3.3, 0.8) {SSTable dirs\\L0 -- L3};
\node[fs] (mani) at (-1.1, 0.8) {\texttt{manifest.json}};
\node[fs] (cfg)  at ( 1.1, 0.8) {\texttt{config.json}};
\node[fs] (clog) at ( 3.3, 0.8) {\texttt{compaction.log}};
\node[fs] (lg)   at ( 5.5, 0.8) {\texttt{logs/}};

% ── Right-side band labels
\node[band] at (7.3, 9)   {clients};
\node[band] at (7.3, 7.3) {HTTP / WS};
\node[band] at (7.3, 5.6) {engine};
\node[band] at (7.3, 4)   {managers};
\node[band] at (7.3, 2.4) {workers};
\node[band] at (7.3, 0.8) {persistence};

% ── Arrows: clients → API (and embedded → engine direct)
\draw[arrk] (repl.south)   -- (repl.south   |- fastapi.north);
\draw[arrk] (spa.south)    -- (spa.south    |- fastapi.north);
\draw[arrk] (httpc.south)  -- (httpc.south  |- fastapi.north);
\draw[arrk] (embed.south)  to[out=-90, in=30] ([xshift=3cm]engine.north);

% ── API → Engine
\draw[arrk] (fastapi.south) -- (engine.north);

% ── Engine → each Manager
\foreach \m in {walmgr, memmgr, sstmgr, flushp, compmgr, conf}
    \draw[arr] (engine.south) -- (\m.north);

% ── Managers → workers
\foreach \m/\w in {walmgr/walw, memmgr/skip, sstmgr/sstrw, compmgr/merge}
    \draw[arr] (\m.south) -- (\w.north);
\draw[arr] (sstmgr.south) -- (bloom.north);
\draw[arr] (sstmgr.south) -- (cache.north);
\draw[arr] (flushp.south) -- (sstrw.north east);

% ── Workers → filesystem (main persistence edges)
\draw[arr] (walw.south)  -- (wal.north);
\draw[arr] (sstrw.south) -- (ssts.north);
\draw[arr] (sstrw.south) -- (mani.north);
\end{tikzpicture}%
}
\caption{KiwiDB System Architecture.}
\label{fig:sysarch}
\end{figure}

\begin{itemize}[leftmargin=2em]
      \item \textbf{Clients.} External consumers of the engine --- the CLI REPL, the React web dashboard, arbitrary HTTP clients, and embedded Python applications that \texttt{await} the engine directly as a library.
      \item \textbf{Presentation (HTTP / WebSocket).} A FastAPI + Uvicorn ASGI process exposes the engine over eight REST routers and a WebSocket stream for live logs, translating network requests into async engine calls.
      \item \textbf{Engine (\texttt{LSMEngine}).} A thin async coordinator that owns the public API (\texttt{put}, \texttt{get}, \texttt{delete}, \texttt{flush}, \texttt{close}) and orchestrates sequence numbering, durability, memtable writes, and background work across its managers.
      \item \textbf{Managers.} Five subsystem managers --- WAL, MemTable, SSTable, Flush Pipeline, Compaction --- each own a single concern and hide their internal state behind a focused async interface consumed by the engine.
      \item \textbf{Workers.} Low-level data structures and file formats: the concurrent skip list, WAL writer, SSTable reader/writer, bloom filter and sparse index, block cache, and k-way merge iterator that perform the actual read, write, and merge work.
      \item \textbf{Persistence.} The local filesystem holds the durable state --- \texttt{wal.log}, per-level SSTable directories (L0--L3), \texttt{manifest.json}, \texttt{config.json}, the compaction audit log, and structured log files.
  \end{itemize}

% \subsection{Layered Dependency Model}

% The stack in Figure~\ref{fig:sysarch} is enforced at the module level by a strict seven-tier dependency hierarchy: no module imports from a tier above it, preventing circular dependencies and enabling each tier to be tested in isolation. Figure~\ref{fig:arch} illustrates this module-level view.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    tier/.style={draw, rounded corners, minimum width=11cm, minimum height=0.5cm, font=\footnotesize, fill=#1},
...
\centering
\begin{tikzpicture}[
    proc/.style={draw, circle, minimum size=1.5cm, font=\scriptsize, align=center, fill=blue!5},
    store/.style={draw, rectangle, minimum width=2cm, minimum height=0.5cm, font=\scriptsize, fill=gray!10},
    entity/.style={draw, rectangle, minimum width=1.5cm, minimum height=0.7cm, font=\scriptsize\bfseries},
    arr/.style={-{Stealth[length=1.5mm]}, thick, font=\scriptsize},
]
...
\draw[arr] (mem) -- node[above] {freeze} (flush);
\draw[arr] (flush) -- node[left] {SSTable} (sst);
...
\begin{figure}[H]
  ...
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
      proc/.style={draw, circle, minimum size=1.3cm, font=\scriptsize, align=center, fill=blue!5},
      store/.style={draw, rectangle, minimum width=1.8cm, minimum height=0.4cm, font=\scriptsize, fill=gray!10},
      arr/.style={-{Stealth[length=1.5mm]}, thick, font=\scriptsize},
  ]
  % Processes arranged horizontally
  \node[proc] (seq)    at (0,    1) {Seq\\Generator};
  \node[proc] (wal)    at (2.8,  1) {WAL\\Append};
  \node[proc] (skip)   at (5.6,  1) {SkipList\\Insert};
  \node[proc] (freeze) at (8.4,  1) {Freeze\\Check};
  \node[proc] (signal) at (11.2, 1) {Signal\\Flush};

  % Data stores below their processes
  \node[store] (seqstore) at (0,   -1.1) {Seq Counter};
  \node[store] (walfile)  at (2.8, -1.1) {WAL File};
  \node[store] (memstore) at (5.6, -1.1) {Active MemTable};

  % External input
  \draw[arr] (-1.8, 1) -- node[above] {put(k,v)} (seq);

  % Process-to-process flow
  \draw[arr] (seq)    -- node[above] {seq num}   (wal);
  \draw[arr] (wal)    -- node[above] {fsync ok}  (skip);
  \draw[arr] (skip)   -- node[above] {inserted}  (freeze);
  \draw[arr] (freeze) -- node[above] {threshold} (signal);

  % Process-to-store flow (side effects)
  ...
  \label{fig:dfd2-write}
\end{figure}

\subsection{DFD Level 2 --- Read Path}
...
\begin{figure}[H]
  ...
  % Processes arranged horizontally
  \node[proc] (active) at (0,    1) {Active\\MemTable};
  \node[proc] (imm)    at (2.8,  1) {Immutable\\Queue};
  \node[proc] (sst)    at (5.6,  1) {SSTable\\Search};
  \node[proc] (tomb)   at (8.4,  1) {Tombstone\\Check};
  \node[proc] (ret)    at (11.2, 1) {Return\\Value};

  % Data stores below their reading processes
  \node[store] (activestore) at (0,   -1.1) {Active MemTable};
  \node[store] (immstore)    at (2.8, -1.1) {Immutable Queue};
  \node[store] (sststore)    at (5.6, -1.1) {SSTable Files};

  % External input
  \draw[arr] (-1.8, 1) -- node[above] {get(k)} (active);

  % Main hit trunk: Active MemTable up → across → down into Tombstone Check
  \draw[arr] (active.north) -- node[right, font=\scriptsize] {hit} (0, 3)
                            -- (8.4, 3)
                            -- (tomb.north);

  % Tributaries joining the trunk from Immutable Queue and SSTable Search
  \draw[arr] (imm.north) -- node[right, font=\scriptsize] {hit} (2.8, 3);
  \draw[arr] (sst.north) -- node[right, font=\scriptsize] {hit} (5.6, 3);

  % Miss flow falls through horizontally to the next lookup layer
  \draw[arr] (active) -- node[above] {miss} (imm);
  \draw[arr] (imm)    -- node[above] {miss} (sst);

  \draw[arr] (sst.east) -- (7.0, 1)  
        -- (7.0, -0.2)
        -- node[below, font=\scriptsize] {miss(value = None)} (11.2, -0.2)
        -- (ret.south); 

  % Tombstone → Return
  \draw[arr] (tomb) -- node[above] {value / None} (ret);

  % Store-to-process flow (reads)
  \draw[arr] (activestore) -- (active);
  \draw[arr] (immstore)    -- (imm);
  \draw[arr] (sststore)    -- (sst);
  \end{tikzpicture}%
  }
  \label{fig:dfd2-read}
  \caption{DFD Level 2 --- Read path detail.}
\end{figure}
...
\centering
\begin{tikzpicture}[
    usecase/.style={draw, ellipse, minimum width=2.6cm, minimum height=0.7cm, font=\scriptsize, align=center},
    arr/.style={-, thick},
]
% System boundary
\draw[thick, rounded corners] (0, -2.6) rectangle (7, 3);
\node[font=\footnotesize\bfseries] at (3.5, 3.3) {KiwiDB System};

% ── User actor (left) ──
\coordinate (userhand) at (-1.4, 0.2);
\draw (-1.9, 0.7) circle (0.17);
\draw (-1.9, 0.53) -- (-1.9, 0.05);
\draw (-2.2, 0.3)  -- (-1.6, 0.3);
\draw (-1.9, 0.05) -- (-2.1, -0.45);
\draw (-1.9, 0.05) -- (-1.7, -0.45);
\node[font=\tiny] at (-1.9, -0.7) {User};

% ── Scheduler actor (right, external timer / background trigger) ──
\coordinate (syshand) at (8.4, 0.2);
\draw (8.9, 0.7) circle (0.17);
\draw (8.9, 0.53) -- (8.9, 0.05);
\draw (8.6, 0.3)  -- (9.2, 0.3);
\draw (8.9, 0.05) -- (8.7, -0.45);
\draw (8.9, 0.05) -- (9.1, -0.45);
\node[font=\tiny, align=center] at (8.9, -0.8) {Scheduler\\(auto)};

% ── Use cases: 2 columns ──
\node[usecase] (put)     at (1.75,  2.2)  {Put Key-Value};
\node[usecase] (get)     at (1.75,  1.2)  {Get Value};
\node[usecase] (del)     at (1.75,  0.2)  {Delete Key};
\node[usecase] (flush)   at (1.75, -0.8)  {Flush MemTable};
\node[usecase] (stats)   at (1.75, -1.8)  {View Stats};

\node[usecase] (browse)  at (5.25,  1.7)  {Browse SSTables};
\node[usecase] (logs)    at (5.25,  0.7)  {View Live Logs};
\node[usecase] (config)  at (5.25, -0.3)  {Configure Engine};
\node[usecase] (compact) at (5.25, -1.3)  {Run Compaction};

% ── User connections (single fan-out anchor) ──
\draw[arr] (userhand) -- (put.west);
\draw[arr] (userhand) -- (get.west);
\draw[arr] (userhand) -- (del.west);
\draw[arr] (userhand) -- (flush.west);
\draw[arr] (userhand) -- (stats.west);
\draw[arr] (userhand) to[bend left=12]  (browse.west);
\draw[arr] (userhand) to[bend left=6]   (logs.west);
\draw[arr] (userhand) to[bend right=8]  (config.west);

% ── Scheduler connections ──
\draw[arr] (syshand) -- (flush.east);
\draw[arr] (syshand) -- (compact.east);
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    decision/.style={draw, diamond, minimum width=1.4cm, minimum height=1.4cm, font=\scriptsize, inner sep=0pt},
...
\node[font=\scriptsize] at (0, 2.8) {threshold?};
\node[state=green!15] (freeze) at (2.8, 1.6) {Freeze memtable};
\node[state=green!15] (signal) at (2.8, 0.6) {Signal flush pipeline};
\node[state=orange!15] (rel) at (0, -0.6) {Release write\_lock};
\node[startend, fill=black, draw=black, double=white] (endd) at (0, -1.6) {};
...
\draw[arr] (wal) -- (ins);
\draw[arr] (ins) -- (check);
\draw[arr] (check) -- node[right, font=\scriptsize, pos=0.4] {yes} (freeze);
\draw[arr] (check) -- node[left, font=\scriptsize] {no} (rel);
\draw[arr] (freeze) -- (signal);
\draw[arr] (signal.south) |- (rel.east);
\draw[arr] (rel) -- (endd);
\end{tikzpicture}
...
\subsection{Activity Diagram --- Read Path}

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    state/.style={draw, rounded corners, minimum width=3.8cm, minimum height=0.5cm, font=\scriptsize, fill=#1, align=center, inner sep=2pt},
    decision/.style={draw, diamond, aspect=2, minimum width=1.5cm, font=\scriptsize, inner sep=1pt, align=center},
    startend/.style={draw, circle, fill=black, minimum size=5pt},
    arr/.style={-{Stealth[length=1.5mm]}, thick},
]
% Main vertical flow (compressed: all SSTable levels into one node)
\node[startend] (start) at (0, 5.6) {};
\node[state=orange!15] (active) at (0, 4.9) {\quad Active MemTable lookup \quad};
\node[state=orange!15] (imm)    at (0, 4.2) {\quad Immutable Queue (newest first) \quad};
\node[state=gray!20]   (sst)    at (0, 3.4) {\quad SSTable Search: L0 $\to$ L1 $\to$ L2 $\to$ L3 \ (bloom $\to$ index $\to$ block) \quad};
\node[decision]        (found)  at (0, 2.5) {found?};
\node[decision]        (tomb)   at (2.9, 1.4) {tombstone?};
\node[state=blue!10, minimum width=2.3cm]  (retnull) at (2.9, 0.2)  {\quad return \texttt{None} \quad};
\node[state=green!15, minimum width=2.3cm] (retval)  at (5.7, 1.4)  {\quad return value \quad};
\node[state=blue!10, minimum width=2.9cm]  (miss)    at (-3.0, 1.4) {\quad return \texttt{None} (not found) \quad};
\node[startend, fill=black, double=white] (endd) at (0, -0.7) {};

% Main flow arrows
\draw[arr] (start) -- (active);
\draw[arr] (active) -- (imm);
\draw[arr] (imm) -- (sst);
\draw[arr] (sst) -- (found);

% Found decision branches
\draw[arr] (found.east) -- node[above, font=\scriptsize] {yes} (tomb.west);
\draw[arr] (found.west) -- node[above, font=\scriptsize] {no}  (miss.east);

% Tombstone decision branches
\draw[arr] (tomb) -- node[right, font=\scriptsize] {yes} (retnull);
\draw[arr] (tomb.east) -- node[above, font=\scriptsize] {no} (retval.west);

% Converge to end
\draw[arr] (retnull) |- (endd);
\draw[arr] (miss) |- (endd);
\draw[arr] (retval) |- (endd);
\end{tikzpicture}
\caption{Activity Diagram --- Read Path.}
\label{fig:activity-read}
\end{figure}
...
\begin{tikzpicture}[
    ...
    obj/.style={draw, rectangle, minimum width=1.3cm, minimum height=0.5cm, font=\scriptsize, fill=yellow!10},
]
\node[obj] (client) at (0, 0) {Client};
\node[obj] (engine) at (2.5, 0) {LSMEngine};
\node[obj] (walmgr) at (5, 0) {WALMgr};
\node[obj] (memmgr) at (7.5, 0) {MemMgr};
\node[obj] (flushp) at (10, 0) {FlushPipeline};
...
\draw[arr, dashed] (7.5, -2.3) -- node[below] {ok} (2.5, -2.3);
...
\draw[arr, dashed] (7.5, -3.9) -- node[below] {(seq,val)} (2.5, -3.9);
...
\draw[arr] (10, -5.5) -- node[above, font=\scriptsize] {write SSTable} (10, -6.8);
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    class/.style={draw, rectangle, rounded corners=2pt, minimum width=2.4cm, minimum height=1.2cm, font=\footnotesize, align=center, fill=#1, inner sep=3pt},
    abc/.style={class=yellow!20},
    engine/.style={class=orange!25, very thick},
    mgr/.style={class=blue!12},
    mem/.style={class=green!15},
    leaf/.style={class=gray!18},
    inh/.style={-{Triangle[length=3mm, open]}, thick, black},
    comp/.style={-{Diamond[length=5mm, open]}, thick, gray!55!black},
]

% ── Row 1: ABCs (y=9) ─────────────────────────────────────
\node[abc] (se)  at (0, 9) {\textbf{Storage}\\\textbf{Engine}};
\node[abc] (mt)  at (3.9, 9) {\textbf{MemTable}};
\node[abc] (ser) at (10.5, 9) {\textbf{Serializable}};

% ── Row 2: Engine + ABC subclasses (y=6.8) ────────────────
\node[engine] (lsm) at (0, 6.8) {\textbf{LSM}\\\textbf{Engine}};
\node[mem] (active) at (3.9, 6.8) {\textbf{Active}\\\textbf{MemTable}};
\node[leaf] (bloom)  at (9.1, 6.8) {\textbf{Bloom}\\\textbf{Filter}};
\node[leaf] (sparse) at (11.7, 6.8) {\textbf{Sparse}\\\textbf{Index}};

% ── Row 3: Managers (y=4) ─────────────────────────────────
\node[mgr] (walmgr)  at (-1.3, 4) {\textbf{WAL}\\\textbf{Manager}};
\node[mgr] (memmgr)  at (1.3,  4) {\textbf{MemTable}\\\textbf{Manager}};
\node[mgr] (sstmgr)  at (6.5,  4) {\textbf{SSTable}\\\textbf{Manager}};
\node[mgr] (flushp)  at (9.1,  4) {\textbf{Flush}\\\textbf{Pipeline}};
\node[mgr] (compmgr) at (11.7, 4) {\textbf{Compaction}\\\textbf{Manager}};

% ── Row 4: Workers (y=1.2) ────────────────────────────────
\node[leaf] (walw)  at (-1.3, 1.2) {\textbf{WAL}\\\textbf{Writer}};
\node[mem]  (immut) at (1.3,  1.2) {\textbf{Immutable}\\\textbf{MemTable}};
\node[mem]  (skip)  at (3.9,  1.2) {\textbf{SkipList}};
\node[leaf] (sstw)  at (11.7, 1.2) {\textbf{SSTable}\\\textbf{Writer}};
\node[leaf] (sstr)  at (6.5,  1.2) {\textbf{SSTable}\\\textbf{Reader}};
\node[leaf] (cache) at (9.1,  1.2) {\textbf{Block}\\\textbf{Cache}};

% ── Inheritance (hollow triangle, solid line) ────────────
\draw[inh] (lsm.north)    -- (se.south);
\draw[inh] (active.north) -- (mt.south);
\draw[inh] (bloom.north)  -- (ser.south);
\draw[inh] (sparse.north) -- (ser.south);

% ── Composition: LSMEngine → 5 managers ──────────────────
\draw[comp] (lsm.south) -- (walmgr.north);
\draw[comp] (lsm.south) -- (memmgr.north);
\draw[comp] (lsm.south) -- (sstmgr.north);
\draw[comp] (lsm.south) -- (flushp.north);
\draw[comp] (lsm.south) -- (compmgr.north);

% ── Composition: Managers → workers ──────────────────────
\draw[comp] (walmgr.south)  -- (walw.north);
\draw[comp] (memmgr.south)  -- (immut.north);
\draw[comp] (memmgr.north) -- (active.west);   % memmgr has active
\draw[comp] (active.south)  -- (skip.north);
\draw[comp] (sstmgr.south)  -- (sstr.north);
\draw[comp] (sstmgr.south)  -- (cache.north);
\draw[comp] (flushp.south)  -- (sstw.north);

% ── SSTableWriter aggregates Bloom + SparseIndex ─────────
\draw[comp] (sstw.north) to[bend right=8]  (bloom.south);
\draw[comp] (sstw.north) to[bend right=50] (sparse.south);
\end{tikzpicture}%
}
...
\centering
\begin{tikzpicture}[
    comp/.style={draw, rectangle, minimum width=2.2cm, minimum height=0.8cm, font=\footnotesize, fill=#1, align=center},
    arr/.style={-{Stealth[length=2mm]}, thick},
...
\begin{figure}[H]
  ...
  \ganttbar{WAL + MemTable Impl.}{4}{7} \\
  \ganttbar{SSTable Impl.}{6}{9} \\
  \ganttbar{Flush Pipeline Impl.}{8}{10} \\
  \ganttbar{Compaction Impl.}{9}{11} \\
  \ganttbar{Cache + Bloom Impl.}{10}{12} \\
  \ganttbar{Web Dashboard Impl.}{11}{13} \\
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Chaitanya Paraskar:} Engine core, architecture design, MemTables, SSTables, Compaction engine.
    \item \textbf{Shreyash Dhas:} REPL Loop, bloom filter, sparse index.
    \item \textbf{Shreyash Gajbhiye:} REST API, flush pipeline, block cache.
...
\node[font=\scriptsize, anchor=east] at (-0.6, 1.8) {Slot 0};
\node[phase, fill=blue!15, minimum width=3.5cm] (w0) at (1.75, 1.8) {write snap0};
\node[phase, fill=green!15, minimum width=1.5cm] (c0) at (4.3, 1.8) {commit};
\node[font=\scriptsize, anchor=east] at (-0.6, 0.8) {Slot 1};
\node[phase, fill=blue!15, minimum width=2.0cm] (w1) at (1.0, 0.8) {write snap1};
\node[phase, fill=yellow!20, minimum width=3cm] (wait1) at (3.5, 0.8) {wait};
\node[phase, fill=green!15, minimum width=1.5cm] (c1) at (5.8, 0.8) {commit};
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Static Type Analysis:} mypy and basedpyright run as dual type checkers.
...
\section{Project Documentation Website}
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Short link:} \url{https://bit.ly/cvp-kiwidb}
\end{itemize}

% Figure~\ref{fig:qr-docs} provides a QR code to the documentation site for quick access from a mobile device.

\begin{figure}[H]
\centering
\includegraphics[width=0.3\textwidth]{QR_kiwi-db_Docs.png}
\caption{QR for project documentation (\url{https://bit.ly/cvp-kiwidb}).}
\label{fig:qr-docs}
\end{figure}

\section{Screenshots}

% ── Helper: uniform screenshot placeholder box ──
\newcommand{\screenshotplaceholder}[1]{%
    \fbox{%
        \begin{minipage}[c][5cm][c]{0.78\textwidth}
            \centering
            \textit{[Screenshot placeholder]}\\[0.6em]
            \small #1
        \end{minipage}%
    }%
}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/dashboard-homepage.png}
% \screenshotplaceholder{Web Dashboard --- Home page showing engine status, live stats,\\and navigation to all 10 inspector pages.}
\caption{Web Dashboard --- Home page.}
\label{fig:screen-dashboard-home}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/terminal.png}
% \screenshotplaceholder{KV Explorer --- interactive page for \texttt{put} / \texttt{get} / \texttt{delete}\\operations with inline response trace.}
\caption{Terminal --- REPL Loop.}
\label{fig:screen-kv-explorer}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/memtable.png}
% \screenshotplaceholder{MemTable Viewer --- live inspection of the active memtable\\and the immutable queue snapshots.}
\caption{Web Dashboard --- MemTable Viewer page.}
\label{fig:screen-memtable-viewer}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/SSTables.png}
% \screenshotplaceholder{SSTable Browser --- per-level listing of SSTables with\\bloom filter, sparse index, and block-level detail views.}
\caption{Web Dashboard --- SSTable Browser page.}
\label{fig:screen-sstable-browser}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/logs.png}
% \screenshotplaceholder{Live Log Stream --- structured log events broadcast over\\WebSocket from the engine to the browser in real time.}
\caption{Web Dashboard --- Live Log Stream page.}
\label{fig:screen-live-logs}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/lookup-trace.png}
% \screenshotplaceholder{In-Browser Terminal --- REPL commands (\texttt{put}, \texttt{get},\\\texttt{trace}, \texttt{mem}, \texttt{disk}, etc.) executed from the web UI.}
\caption{Web Dashboard --- Lookup-Trace page.}
\label{fig:lookup}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/config.png}
% \screenshotplaceholder{Configuration Page --- live editing of runtime parameters\\(memtable thresholds, bloom FPR, compaction triggers, etc.).}
\caption{Web Dashboard --- Configuration page.}
\label{fig:screen-config}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/load-generator.png}
% \screenshotplaceholder{Stats / Write-Amplification Page --- engine counters,\\historical ring buffer, and measured write-amp ratio.}
\caption{Web Dashboard --- Load-Generator.}
\label{fig:load-gen}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{screenshots/docs.png}
% \screenshotplaceholder{Pytest Output --- full test suite run (211+ tests across\\25 modules) reporting all-pass status.}
...
As part of the dissemination effort, four research papers --- three implementation papers and one survey paper --- have been prepared and submitted to peer-reviewed venues. At the time of writing, all submissions are under review; none have been accepted yet.

\vspace{0.5em}
\noindent\textbf{Implementation Paper (Submission~1) --- ICCUBEA.}\\
\textbf{Title:} ``KiwiDB: An Async-Native Log-Structured Merge Tree Key-Value Store''.\\
\textbf{Venue:} International Conference on Computing, Communication, Control and Automation (ICCUBEA), PCCOE Pune --- \url{https://iccubea.pccoepune.com/}.\\
\textbf{Status:} Submitted; under review.\\
\textbf{Scope:} Presents the system architecture, design innovations, and implementation details of KiwiDB, with a head-to-head comparison against LevelDB and RocksDB.

\vspace{0.5em}
\noindent\textbf{Implementation Paper (Submission~2) --- ICIICS.}\\
\textbf{Title:} ``KiwiDB: An Async-Native Log-Structured Merge Tree Key-Value Store''.\\
\textbf{Venue:} International Conference on Innovations in Intelligent Computing and Sustainability (ICIICS) --- \url{https://iciics.in/}.\\
\textbf{Status:} Submitted; under review.

\vspace{0.5em}
\noindent\textbf{Implementation Paper (Submission~3) --- IC3.}\\
\textbf{Title:} ``KiwiDB: An Async-Native Log-Structured Merge Tree Key-Value Store''.\\
\textbf{Venue:} International Conference on Contemporary Computing (IC3) --- \url{https://www.ic3conf.net/}.\\
\textbf{Status:} Submitted; under review.

\vspace{0.5em}
\noindent\textbf{Survey Paper --- IJERT.}\\
\textbf{Title:} ``Survey of LSM Trees and Indexing Techniques in Key-Value Stores''.\\
\textbf{Venue:} International Journal of Engineering Research \& Technology (IJERT) --- \url{https://www.ijert.org/call-for-papers}.\\
\textbf{Status:} Submitted; under review.\\
\textbf{Scope:} Provides a structured overview of LSM-tree-based storage architectures with a focus on indexing mechanisms and optimization strategies for primary and secondary key access. Covers core components (memtables, write-ahead logs, SSTables, Bloom filters, sparse indexes), surveys representative systems (LevelDB, RocksDB, Cassandra, HBase, AsterixDB), and reviews emerging adaptations involving persistent memory, disaggregated memory systems, hybrid storage layouts, and key-value separation.
...
\begin{center}
    {\fontsize{15}{17}\selectfont \textbf{\underline{SCTR's Pune Institute of Computer Technology, Pune}}}\\[0.5em]
    {\fontsize{14}{15}\selectfont \underline{Department of Computer Engineering}}
\end{center}

\vspace*{5\baselineskip}

\begin{center}
    {\fontsize{15}{16}\selectfont \textbf{\underline{UNDERTAKING BY STUDENT}}}
\end{center}

\vspace*{2\baselineskip}

\setlength{\parindent}{5em}
We, the students of B.E.\ Computer Engineering, Academic year 2025--26, hereby declare that the AI tools like ChatGPT are not used for developing the solution for the project entitled \textbf{``High Throughput Write-Optimised Key-Value Store using LSM Tree Engine''}.
...
\noindent
\begin{tabular}{@{}p{8cm}@{\hspace{0.5cm}}p{5cm}@{}}
\textbf{Name of the student}                               & \multicolumn{1}{c}{\textbf{Signature}} \\[2em]
1.~Chaitanya Paraskar                                      & \rule[0pt]{5cm}{0.4pt} \\[1.5em]
2.~Shreyash Dhas                                           & \rule[0pt]{5cm}{0.4pt} \\[1.5em]
3.~Shreyash Gajbhiye                                       & \rule[0pt]{5cm}{0.4pt} \\[1.5em]
4.~Shreyas Gopnarayan                                      & \rule[0pt]{5cm}{0.4pt} \\[1.5em]
\end{tabular}
...
\noindent
\begin{tabular}{@{}p{8cm}@{\hspace{0.5cm}}p{5cm}@{}}
\textbf{Name of the Guide}                                 & \multicolumn{1}{c}{\textbf{Signature}} \\[2em]
Prof.\ M. V. Mane                                          & \rule[0pt]{5cm}{0.4pt} \\
\end{tabular}
