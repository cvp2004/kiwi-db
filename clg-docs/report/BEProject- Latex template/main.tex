...
This section presents screenshots captured from the running KiwiDB application --- the web dashboard pages, the interactive CLI REPL, the automated test suite, and the static type checker output. Each placeholder below is to be replaced with the corresponding captured image at submission time.
...
\caption{Web Dashboard --- KV Explorer page.}
...
\begin{figure}[H]
\centering
\screenshotplaceholder{Compaction Monitor --- active compaction jobs, level\\reservations, and the compaction audit history.}
\caption{Web Dashboard --- Compaction Monitor page.}
\label{fig:screen-compaction-monitor}
\end{figure}
...
\caption{Web Dashboard --- In-Browser Terminal page.}
\label{fig:screen-terminal}
\end{figure}
...
\caption{Web Dashboard --- Stats and Write-Amplification page.}
\label{fig:screen-stats}
\end{figure}

\begin{figure}[H]
\centering
\screenshotplaceholder{CLI REPL Session --- interactive prompt-toolkit session\\showing \texttt{put} / \texttt{get} / \texttt{trace} / \texttt{flush} command output.}
\caption{CLI REPL --- interactive session.}
\label{fig:screen-repl}
\end{figure}

\begin{figure}[H]
\centering
\screenshotplaceholder{Pytest Output --- full test suite run (211+ tests across\\25 modules) reporting all-pass status.}
\caption{Automated test suite --- \texttt{pytest} output.}
\label{fig:screen-pytest}
\end{figure}

\begin{figure}[H]
\centering
\screenshotplaceholder{Static Type Checker Output --- \texttt{mypy --strict} and\\\texttt{basedpyright --strict} reporting zero errors across all modules.}
\caption{Static type checking --- \texttt{mypy} and \texttt{basedpyright} output.}
\label{fig:screen-typecheck}
\end{figure}
